#!/usr/bin/env python3
#
# STC32 MCS-251 ISA integration test
#
...
class STC32ISATest(QemuSystemTest):
    FLASH_BASE = 0xfc2800
    FLASH_SIZE = 246 * 1024
    RESET_PC = 0xff0000
    ...
    def build_firmware(self):
        ...
        path = Path(self.scratch_file('stc32-isa.bin'))
    ...
    def build_classic_firmware(self):
        ...
        p.emit(0xd2, 0x00)
        p.emit(0xa2, 0x00)
        p.branch(0x40, 'classic_carry_set')
        p.ljmp('fail')
        p.label('classic_carry_set')
        p.emit(0x92, 0x01)
        p.emit(0xb2, 0x00)
        p.classic_bit_branch(0x30, 0x00, 'classic_bit_clear')
        p.ljmp('fail')
        p.label('classic_bit_clear')
        p.emit(0xd2, 0x02)
        p.classic_bit_branch(0x10, 0x02, 'classic_jbc')
        p.ljmp('fail')
        p.label('classic_jbc')
        p.classic_bit_branch(0x30, 0x02, 'classic_jbc_clear')
        p.ljmp('fail')
        p.label('classic_jbc_clear')
        p.emit(0xc3)
        p.emit(0xa0, 0x00)
        p.branch(0x40, 'classic_or_not_bit')
        p.ljmp('fail')
        p.label('classic_or_not_bit')
        p.emit(0xd2, 0x00)
        p.emit(0xb0, 0x00)
        p.branch(0x50, 'classic_and_not_bit')
        p.ljmp('fail')
        p.label('classic_and_not_bit')

        p.emit(0x74, 0x00)
        p.branch(0x60, 'classic_jz')
        p.ljmp('fail')
        p.label('classic_jz')
        p.emit(0x74, 0x01)
        p.branch(0x70, 'classic_jnz')
        p.ljmp('fail')
        p.label('classic_jnz')
        ...
        image[self.RESET_PC - self.FLASH_BASE:
              self.RESET_PC - self.FLASH_BASE + len(code)] = code
        path = Path(self.scratch_file('stc32-classic-isa.bin'))
        path.write_bytes(image)
        return path
    ...
    def build_native_firmware(self):
        ...
        p.bit_branch(0x30, 3, 0x2a, 'native_cpl_bit')
        p.jump('fail')
        p.label('native_cpl_bit')
        p.emit(0xc3)
        ...
        p.bit_branch(0x10, 1, 0x2a, 'native_jbc')
        p.jump('fail')
        p.label('native_jbc')
        p.bit_branch(0x30, 1, 0x2a, 'native_jbc_clear')
        p.jump('fail')
        p.label('native_jbc_clear')
        ...
        self.mov_imm(p, 0, 0x01)
        self.mov_imm(p, 1, 0x02)
        p.emit(0xbc, 0x01)
        p.branch(0x48, 'native_signed_less')
        p.jump('fail')
        p.label('native_signed_less')
        p.branch(0x28, 'native_unsigned_less')
        p.jump('fail')
        p.label('native_unsigned_less')
        p.emit(0xbc, 0x00)
        p.branch(0x68, 'native_equal')
        p.jump('fail')
        p.label('native_equal')
        self.mov_imm(p, 0, 0x02)
        self.mov_imm(p, 1, 0x01)
        p.emit(0xbc, 0x01)
        p.branch(0x38, 'native_unsigned_greater')
        p.jump('fail')
        p.label('native_unsigned_greater')
        ...
        image[self.RESET_PC - self.FLASH_BASE:
              self.RESET_PC - self.FLASH_BASE + len(code)] = code
        path = Path(self.scratch_file('stc32-native-isa.bin'))
        path.write_bytes(image)
        return path
    ...
    def test_classic_instruction_families(self):
    ...
    def test_native_instruction_families(self):
